...
def _keyboard_layout_items() -> list[DropdownItem]:
    layouts = parse_xkb_layouts()
    items: list[DropdownItem] = []
    for code, name in sorted(layouts.items()):
        label = f"{name} ({code})" if name and name != code else code
        items.append(DropdownItem(code, label))

    # Ensure common defaults are present even if parse fails
    defaults = [("us", "English (US)"), ("tr", "Turkish"), ("gb", "English (UK)")]
    present = {i.value for i in items}
    for code, label in defaults:
        if code not in present:
            items.insert(0, DropdownItem(code, label))
...
class InputPage(gtk.ScrolledWindow):
    ...
    def __init__(self) -> None:
        ...
        self.box_children = (
            Category("Keyboard"),
            SettingsDropdownRow(
                "Keyboard layout (single)",
                "Pick from detected layouts; use the field below for multiple/custom",
                "input.kb_layout",
                items=_keyboard_layout_items(),
            ),
            Hint("Multiple layouts? Use comma-separated codes in the text field below."),
            SettingsTextRow(
                "Keyboard model",
