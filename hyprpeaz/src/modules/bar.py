...
class KeyboardLayout(gtk.Label):
    ...
    def __init__(self) -> None:
        ...
        self.click_gesture = gtk.GestureClick.new()
        self.click_gesture.set_button(0)
        self.gesture_conn = self.click_gesture.connect(
            "released", self.on_click
        )
        self.add_controller(self.click_gesture)
    ...
    def on_click(self, *args: t.Any) -> None:
        """Cycle through available layouts using hyprctl."""
        try:
            proc = subprocess.run(
                ["hyprctl", "-j", "devices"],
                check=True,
            ...
            keyboard = next(
                (kb for kb in keyboards if kb.get("main")), keyboards[0]
            )
            layouts = str(keyboard.get("layout", "")).strip().strip(",").split(",")
            layouts = [l for l in layouts if l]
            if not layouts:
                return

            current = str(keyboard.get("active_keymap", layouts[0]))
            try:
            ...
            device_id = keyboard.get("name") or keyboard.get("address") or ""
            if not device_id:
                return

            subprocess.run(
                ["hyprctl", "switchxkblayout", device_id, str(next_idx)],
                check=False
            )

            # Refresh from hyprctl to get the active_keymap after switch
            try:
                ...
                keyboards2 = data2.get("keyboards") or []
                if isinstance(keyboards2, list):
                    kb2 = next(
                        (kb for kb in keyboards2 if kb.get("main")), keyboards2[0]
                    )
                    new_layout = str(kb2.get("active_keymap", layouts[next_idx]))
                    active_layout.value = new_layout
            ...
            active_layout.value = layouts[next_idx]
            show_layout.value = True
        except Exception:
            # Swallow errors to avoid UI crash if hyprctl unavailable
            return
