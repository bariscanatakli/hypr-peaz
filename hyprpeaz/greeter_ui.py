...
# It's like hyprpeaz_ui.py but minimum version of it
# I like it
...
class GreeterUI(gtk.ApplicationWindow):
    def __init__(
        ...
        self.info_label = gtk.Label(
            halign=gtk.Align.END,
            valign=gtk.Align.START,
            label=f"HyprPeaz v{get_version()}",
...
class HyprPeazGreeter(gtk.Application):
    __gtype_name__ = "HyprPeazGreeter"
...
def main() -> None:
    ...
    app = HyprPeazGreeter(
        application_id="com.bariscanatakli.hyprpeaz.greeter"
    )
    app.run(None)
