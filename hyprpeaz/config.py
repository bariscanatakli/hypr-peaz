...
APP_CACHE_DIR = pjoin(CACHE_DIR, "hyprpeaz")
DATA_HOME = os.getenv("XDG_DATA_HOME", f"{HOME}/.local/share")
LOCAL_ASSETS = pjoin(DATA_HOME, "hyprpeaz", "hyprpeaz-assets")
LOCAL_LIB = pjoin(os.getenv("XDG_LIB_HOME", f"{HOME}/.local/lib"), "hyprpeaz", "hyprpeaz")

ASSETS_DIR = os.getenv(
    "HYPRPEAZ_ASSETS",
    LOCAL_ASSETS if os.path.isdir(LOCAL_ASSETS) else "/usr/share/hyprpeaz"
)
ORIGINAL_DIR = os.getenv(
    "HYPRPEAZ_LIB",
    LOCAL_LIB if os.path.isdir(LOCAL_LIB) else "/usr/lib/hyprpeaz"
)
TEMP_DIR = f"/tmp/hyprpeaz-{os.getenv('USER', 'unknown')}"
...
config_dir = pjoin(CONFIG_DIR, "hyprpeaz")
...
info = {
    "name": "HyprPeaz",
    "author": "bariscanatakli",
    "github": "https://github.com/bariscanatakli/hypr-peaz",
