...
type RegisterType = t.Callable[["HyprPeaz"], type[t.Any] | object]
...
class HyprPeaz(gtk.Application):
    __gtype_name__ = "HyprPeaz"
...
def init() -> None:
    ...
    if is_socket_exists():
        logger.critical(
            "Another HyprPeaz session is running on the same Hyprland instance!"
        )
        exit(1)
...
def main() -> None:
    ...
    app = HyprPeaz(application_id="com.bariscanatakli.hyprpeaz")
    app.run(None)
