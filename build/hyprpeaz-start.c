...
static const char *get_working_dir(void) {
    const char *env_dir = getenv("HYPRPEAZ_LIB");
    if (env_dir && env_dir[0] != '\0') {
        return env_dir;
    }
    return "/home/baris/.local/lib/hyprpeaz/hyprpeaz";
}
...
static void show_crash_dialog(int exit_code)
{
    ...
    execlp("hyprpeaz-crash-dialog", "hyprpeaz-crash-dialog", exit_code_str, NULL);
...
int main(void)
{
    ...
    while (1)
    {
        ...
        char crash_dir[512];
        snprintf(crash_dir, sizeof(crash_dir), "%s/.cache/hyprpeaz/crashes", home_dir);
        ...
        if (pid == 0)
        {
            ...
            setenv("HYPRPEAZ_WATCHDOG", "1", 1);
            close(pipefd[0]);
            dup2(pipefd[1], STDOUT_FILENO);
            dup2(pipefd[1], STDERR_FILENO);
            close(pipefd[1]);
            execlp("python", "python", "-O", "hyprpeaz_ui.py", NULL);
            perror("exec failed");
            exit(127);
        }
